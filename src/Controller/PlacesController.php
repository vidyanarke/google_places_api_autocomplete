...
/**
 * @file
 * Contains \Drupal\places_api_autocomplete\Controller\PlacesController.
 */
...
class PlacesController extends ControllerBase {
  ...
  protected $results;
      ...
      $widget_settings = $entity_type_manager->getStorage('entity_form_display')->load($entity_type . '.' . $bundle . '.' . 'default');

      $specific_widget_type = $widget_settings->getComponent($field_name);

      //kint($specific_widget_type['settings']);
      global $language;
//      error_log($entity_type.' : '.$bundle);
      //kint($field_name);
//      die();
		  $matches = array();
		  $string = $request->query->get('q');
		  
		  $this->key = \Drupal::config('places.settings')->get('places_api_autocomplete_key', '');

		  // If this is not a field, but instead used in a global context, use the
		  // global options.
		  if ($string) {
			  //$options = \Drupal::config('places.settings')->get('places_api_autocomplete_options', places_autocomplete_default_options());
